...
// --- Sub-components extracted to prevent re-render/state loss issues ---

const OverviewContent: React.FC<{ user: Caregiver, setCheckInModalData: any, setSelectedAppointmentForLog: any, setIsSupportModalOpen: any, setViewingAppointment: any }> = ({ user, setCheckInModalData, setSelectedAppointmentForLog, setIsSupportModalOpen, setViewingAppointment }) => {
    const { appointments, updateCaregiverDetails, setVacationMode, addAlert } = useAppStore();
               ...
               <StatCard title="Avaliação Média" value={user.rating.toFixed(1)} color="bg-gradient-to-r from-yellow-500 to-orange-600" icon={<svg className="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>} />
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
                   <div className="flex justify-between items-center mb-4">
                       <h3 className="text-xl font-bold text-gray-800 dark:text-white">Segurança e Confiança</h3>
                       <button onClick={() => setIsSupportModalOpen(true)} className="text-sm text-indigo-600 dark:text-indigo-400 font-semibold hover:underline">Precisa de Ajuda?</button>
                   </div>
           ...
           <div className="mt-8 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
               ...
               <div className="space-y-4">
                   {userAppointments.filter(a => a.status === 'confirmed' || a.status === 'completed').slice(0, 3).map(app => {
                       ...
                       return (
                           <div key={app.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-5 bg-gray-50 dark:bg-gray-700/50 rounded-2xl gap-4 border border-gray-100 dark:border-gray-600 shadow-sm">
                               <div className="w-full">
                                   <div className="flex justify-between items-start mb-2">
                                       <div>
                                           <p className="font-bold text-lg text-gray-800 dark:text-white">{app.clientName}</p>
                                       ...
                                       <button 
                                            ...
                                            className="text-xs bg-indigo-50 text-indigo-600 dark:bg-indigo-900/50 dark:text-indigo-300 px-3 py-1.5 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900 transition-colors font-semibold flex items-center gap-1"
                                       >
                                   ...
                                   <div className="mt-2 text-sm text-gray-600 dark:text-gray-300 space-y-1.5 bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                                       <p className="flex items-center gap-2">
                                           <span className="text-lg">📞</span> <span className="font-medium select-all">{app.clientPhone || 'Não informado'}</span>
                                       </p>
                                       {app.clientStreet ? (
                                           <>
                                               <p className="flex items-start gap-2">
                                                   ...
                                                   <span>
                                                       {app.clientStreet}, {app.clientAddressNumber}
                                                       <br/>
                                                       <span className="text-xs text-gray-500 uppercase font-semibold">{app.clientNeighborhood} - {app.clientCity}/{app.clientState}</span>
                                                   </span>
                                               </p>
                                           </>
                                       ) : (
                                           <p className="flex items-center gap-2 text-gray-400 italic">
                                               <span className="text-lg">📍</span> Endereço não detalhado
                                           </p>
                                       )}
                                   </div>

                                   <div className="mt-2 flex gap-4 text-xs font-medium">
                                       {app.checkIn && <span className="text-green-600 bg-green-100 px-2 py-1 rounded">Entrada: {new Date(app.checkIn.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>}
                                       {app.checkOut && <span className="text-blue-600 bg-blue-100 px-2 py-1 rounded">Saída: {new Date(app.checkOut.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>}
                                   </div>
                               </div>

                               <div className="flex gap-2 flex-wrap justify-start md:justify-end w-full md:w-auto mt-2 md:mt-0">
                                   {canCheckIn && (
                                       <button 
                                   ...
                                   {!app.hasLog && (
                                       <button 
                                           ...
                                           className="px-4 py-2 text-sm bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300 rounded-lg hover:bg-indigo-200 dark:hover:bg-indigo-900 transition-colors font-semibold"
                                       >
                                           Diário
                                       </button>
                                   )}
                                   {app.hasLog && (
                                       <span className="px-4 py-2 text-sm text-green-600 bg-green-100 dark:bg-green-900/30 rounded-lg font-semibold flex items-center gap-1">
                                           Diário ✅
                                       </span>
                                   )}
                               </div>
                   ...
                   <div className="flex items-center bg-gray-100 dark:bg-gray-700 rounded-lg p-2 mb-4">
                    ...
                    <div className="flex justify-center gap-4">
                       ...
                       <a href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(profileUrl)}&title=Conheça%20meu%20perfil%20de%20cuidador%20na%2099Cuidar`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 flex items-center justify-center rounded-full bg-sky-700 text-white hover:bg-sky-800 transition-colors" aria-label="Share on LinkedIn">
                           <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.48 2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.59-11.018-3.714v-2.155z"/></svg>
                       </a>
                   </div>
               </div>
           </div>
...
const AgendaContent: React.FC<{ user: Caregiver, setViewingAppointment: any }> = ({ user, setViewingAppointment }) => {
    const { appointments, updateUserAvailability } = useAppStore();
    ...
    const appointmentsByMonth = useMemo(() => {
        ...
        return userAppointments.filter(app => {
            ...
            // Fix timezone issue by adding offset
    ...
    const renderCalendar = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            // Use UTC/Timezone aware comparison if needed, here just string match
            const hasAppointment = appointmentsByMonth.some(app => app.date === dateString);
    ...
    const selectedDateAppointments = userAppointments.filter(app => app.date === selectedDateString);
    const conflictingAppointmentIds = useMemo(() => 
        findConflictingAppointments(selectedDateAppointments),
    [selectedDateAppointments]);

    const isSelectedDateUnavailable = user.unavailableDates?.includes(selectedDateString);
                        ...
                        return (
                            <div key={app.id} className={`relative p-4 rounded-xl border-l-4 animate-fade-in transition-all ${isConflicting ? 'border-red-500 bg-red-50 dark:bg-red-900/30' : 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30'}`}>
                                ...
                                <div className="flex justify-between items-start">
                                    ...
                                    <button onClick={() => setViewingAppointment(app)} className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 dark:text-indigo-400">Ver Detalhes</button>
                                </div>
                                <div className="mt-2 text-sm text-gray-600 dark:text-gray-400 space-y-1">
                                    ...
                                    <p><span className="font-semibold">Tel:</span> {app.clientPhone || 'N/A'}</p>
                                    <p className="text-xs italic truncate">{app.clientStreet || 'Endereço N/A'}</p>
                                </div>
                            </div>
                        );
                    })}</div>
...
const ServicesContent: React.FC<{ user: Caregiver }> = ({ user }) => {
    const { updateUserServices, addAlert } = useAppStore();
    ...
    const handleGenerateIcon = async (index: number) => {
        ...
        try {
            ...
            console.error(error);
            addAlert('Erro ao gerar ícone. Chave API pode estar faltando.', 'error');
        } finally {
    ...
    const handleGenerateDescription = async (index: number) => {
        ...
        try {
            ...
            console.error(error);
            addAlert('Erro ao gerar descrição. Chave API pode estar faltando.', 'error');
        } finally {
...
const FinancialContent: React.FC<{ user: Caregiver }> = ({ user }) => {
...
const DashboardPage: React.FC = () => {
    ...
    // 1. Define hooks unconditionally at the top
    ...
    // 2. Effect for redirection if needed
    useEffect(() => {
    ...
    // 3. Return null if invalid user to prevent rendering protected content
    if (!currentUser || currentUser.role !== 'caregiver') {
        return null;
    }
    ...
    const renderContent = () => {
        switch(activeTab) {
            case 'overview':
                return <OverviewContent user={user} setCheckInModalData={setCheckInModalData} setSelectedAppointmentForLog={setSelectedAppointmentForLog} setIsSupportModalOpen={setIsSupportModalOpen} setViewingAppointment={setViewingAppointment} />;
            case 'agenda':
                return <AgendaContent user={user} setViewingAppointment={setViewingAppointment} />;
            case 'services':
                return <ServicesContent user={user} />;
            case 'financial':
                return <FinancialContent user={user} />;
            case 'wallet':
                return <WalletPage />;
            case 'settings':
        ...
        <div className="pt-20 min-h-screen bg-gray-50 dark:bg-gray-900">
            ...
            {viewingAppointment && (
                <AppointmentDetailsModal 
                    ...
                    onCheckIn={() => setCheckInModalData({ appointment: viewingAppointment, type: 'in' })}
                    onCheckOut={() => setCheckInModalData({ appointment: viewingAppointment, type: 'out' })}
                />
            )}
        </div>
    );
